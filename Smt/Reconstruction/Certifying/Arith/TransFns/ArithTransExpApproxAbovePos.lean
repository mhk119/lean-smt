...
#check Int.le_self_sq


example (a b c: Real) (hb : 0 ≤ c) (ha : a ≤ b): a*c ≤ b*c := by
  exact mul_le_mul_of_nonneg_right ha hb
...
example (x : Nat) (hx : 2 ∣ x) : 2*(x/2) = x := by
  rw [mul_comm]
  exact Nat.div_mul_cancel hx

example (x : Nat) : x*(1/2) = (x : Real)/2 := by
  linarith
...
theorem pow_lt_factorial' {x : Real} (hx : 0 < x) :
   ∀ d' ≥ (4*(ceil x)^2), x^d' < Nat.factorial d' := by
  have h1 := pow_lt_factorial hx
  intro d' hd'
...
theorem arithTransExpApproxAbovePos (l u t : ℝ) (ht : l ≤ t ∧ t ≤ u) (hl : 0 < l):
  let r : ℕ → ℝ → ℝ := fun d => (fun t => (1-t^(d+1)/(d+1)!))
  let p : ℕ → ℝ → ℝ := fun d => ((taylorWithinEval Real.exp d Set.univ 0) / (r d))
  ∀ d ≥ 4*(ceil u)^2, Real.exp t ≤ ((p d l - p d u) / (l - u)) * (t - l) + p d l := by
  intro r _ d hd
  have h1 := expApproxAbovePos (lt_of_lt_of_le hl (le_trans ht.1 ht.2)) d hd
